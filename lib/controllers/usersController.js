...
export async function index (request, reply) {
  ...
  const pagination = await getUsersPage(page)
...
export async function create (request, reply) {
  ...
  try {
    ...
    setFlashSuccess(request, 'Пользователь добавлен в список.')
    return reply.redirect(request.server.reverse(RouteNames.USERS_INDEX))
  } catch (error) {
    if (error.name === 'ValidationError') {
      setFlashError(request, 'Не удалось добавить пользователя. Исправьте ошибки в форме.')
...
export async function update (request, reply) {
  ...
  try {
    ...
    setFlashSuccess(request, 'Данные пользователя обновлены.')
...
export async function destroy (request, reply) {
  ...
  if (!deleted) {
    setFlashError(request, 'Пользователь не найден.')
    return reply.redirect(request.server.reverse(RouteNames.USERS_INDEX))
  }

  setFlashSuccess(request, 'Пользователь удалён.')
  return reply.redirect(request.server.reverse(RouteNames.USERS_INDEX))
}
